...
public final class AccelerometerListener {
    ...
    public void enable(boolean enable) {
        if (DEBUG) Log.df(TAG, "enable(%s)", enable);
        synchronized (this) {
            if (enable) {
    ...
    private void onSensorEvent(double x, double y, double z) {
        if (VDEBUG) Log.df(TAG, "onSensorEvent(%s, %s, %s)", x, y, z);
        ...
        if (VDEBUG) Log.df(TAG, "angle: %s orientation: %s", angle, orientation);
            ...
            switch (msg.what) {
                case ORIENTATION_CHANGED:
                    synchronized (listener) {
                        ...
                        if (DEBUG) {
                            Log.df(TAG, "orientation: %s",
                                    (((listener.mOrientation) == (ORIENTATION_HORIZONTAL)) ? "horizontal" : (((listener.mOrientation) == (ORIENTATION_VERTICAL)) ? "vertical" : "unknown")));
                        }
                        listener.orientationListener.orientationChanged(listener.mOrientation);
                    }
                    break;
            }
        }
