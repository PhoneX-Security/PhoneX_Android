...
public class RegistrationIntentService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        Log.df(TAG, "onHandleIntent; intent=%s", intent);
        ...
        try {
            ...
            // [END get_token]
            Log.inf(TAG, "GCM Registration Token: " + token);
...
//            sharedPreferences.edit().putBoolean(QuickstartPreferences.SENT_TOKEN_TO_SERVER, true).apply();
            ...
            Log.df(TAG, e, "Failed to complete token refresh");
...
//        LocalBroadcastManager.getInstance(this).sendBroadcast(registrationComplete);
    ...
    private void storeRegistrationToken(String token) {
        // Send token later when we are logged in
        prefs.setString(PhonexConfig.GCM_TOKEN, token);
        prefs.setBoolean(PhonexConfig.GCM_TOKEN_SENT_TO_SERVER, false);
    }
