...
#define DBGOUTV(...) PJ_LOG(5, (THIS_FILE, __VA_ARGS__));
#define DBGOUTD(...) PJ_LOG(4, (THIS_FILE, __VA_ARGS__));
#define DBGOUTI(...) PJ_LOG(3, (THIS_FILE, __VA_ARGS__));
#define DBGOUTW(...) PJ_LOG(2, (THIS_FILE, __VA_ARGS__));
#else
#define DBGOUTV(...)
#define DBGOUTD(...)
#define DBGOUTI(...)
...
#define ZRTP_HASH_STRING "zrtp-hash"
...
typedef struct pjmedia_zrtp_info
{
    ...
    /**
     * Current ZRTP engine state
     */
    pj_uint8_t zrtpState;

    /**
     * MISC fields
     */
    pj_bool_t negotFailed;	/* flag for negotiation failed (ZRTP may fail because ICE check is in progress), restarts ZRTP */
    pj_bool_t starting;		/* to avoid race conditions for ZRTP start, some king of semaphore */
...
/**
 * ZRTP SDP hash verification
 */
typedef enum pjmedia_zrtphash_check
{
	/** Invalid value */
	PJMEDIA_ZRTPHASH_INVALID = 0x0,
	
    /** zrtp-hash from ZRTP Hello packet matches zrtp-hash from SDP request */
    PJMEDIA_ZRTPHASH_SDP_MATCHES_ZRTP = 0x1,

    /** Error, no zrtp-hash in SDP request */
    PJMEDIA_ZRTPHASH_NO_SDP = 0x2,
    
    /** Error, no zrtp-hash in ZRTP negotiation */
    PJMEDIA_ZRTPHASH_NO_ZRTP = 0x4,
    
    /** Error, no zrtp-hash in ZRTP negotiation */
    PJMEDIA_ZRTPHASH_MISMATCH = 0x8
} pjmedia_zrtphash_check;
...
typedef struct zrtp_UserCallbacks
{
    ...
    /**
     * Callback to verify zrtp-hash match in SDP vs. ZRTP committment.
     */
    int32_t (*zrtp_checkZrtpHashMatch)(void* data, int32_t matchResult);

    /**
    * Callback when ZRTP transport is being destroyed.
    * @param data
    *    The pointer user data
    */
    void (*zrtp_transportDestroy)(void* data);
...
/**
 * Returns slave transport that ZRTP uses to transmit its data.
 *
