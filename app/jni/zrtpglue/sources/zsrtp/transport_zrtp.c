...
#include <transport_zrtp_protected.h>
...
//    pj_int32_t sendBufferLen;
    ...
    pj_bool_t volatile started;

    pj_bool_t close_slave;
    pj_bool_t mitmMode;

    // New fields.
    pj_bool_t volatile starting;		/* Flag indicating status of a start lock mutex. */
    pj_mutex_t* zrtpStartingMutex;      /* mutex for locking starting routine */

    pj_bool_t negotFailed;	/* flag for negotiation failed (ZRTP may fail because ICE check is in progress), restarts ZRTP */
    uint64_t rtpUnsec;		/* packet received via RTP in unsecured state */
    uint64_t rtpSec; 		/* packets received via RTP in secured state (ZRTP enabled) */
    uint32_t zrtpCn;		/* number of received ZRTP packets */
    //pj_timer_entry startTimeoutEntry; /* timer entry for postponed ZRTP start due to ICE presence in media */

    pj_bool_t iceBusy;		/* if ICE is in place and is in the middle of negotiating process, we cannot send data */
    pj_uint8_t notSupportedCounter; /* number of not supported calls counter */
    pj_uint8_t peerSDPZrtpHashesSeen; /* number of ZRTP hashes seen in SDP */
    pj_str_t peerSDPZrtpHash[MAX_ZRTP_VERSIONS]; /* zrtp-hash of remote peer sent by SDP session. For verification. Multiple hashes possible. */
    uint8_t  peerSDPZrtpHashCount; /* Number of zrtp-hashes from SDP session. */
    pj_bool_t zrtpHashCheckPostponed; /* was zrtp-hash check postponed to SAS sect. due to hash unavailability? */
};
...
//static void startTimer_callback(pj_timer_heap_t *ht, pj_timer_entry *e);

//                                         1
//                                1234567890123456
static pj_char_t clientId[] =    "PhoneX ZRTP     ";
        ...
        pj_bool_t close_slave)
{
    ...
    zrtp->base.type = (pjmedia_transport_type) (PJMEDIA_TRANSPORT_TYPE_ZRTP);
    zrtp->base.op = &tp_zrtp_op;

    DBGOUTI("ZRTP pjmedia_transport_zrtp_create 364; name: [%s]", name==NULL ? "x" : name);

#ifndef DYNAMIC_TIMER
    rc = zrtp_timer_init(endpt);
    if (rc != PJ_SUCCESS){
    ...
    if (rc != PJ_SUCCESS){
        DBGOUTW("Cannot create mutex, code=%d", rc);
        goto DEALLOC;
    }

    /* zrtp start mutex */
    rc = pj_mutex_create_simple(zrtp->pool, "zrtpStart", &zrtp->zrtpStartingMutex);
    if (rc != PJ_SUCCESS){
        DBGOUTW("Cannot create start mutex, code=%d", rc);
        goto DEALLOC;
    }
    ...
    zrtp->zrtpSeq = 1;                  /* randomize */
    zrtp_genRandomData((uint8_t *)(&zrtp->zrtpSeq), sizeof(zrtp->zrtpSeq)); // randomizing sequence
    ...
    zrtp->refcount++;
    *p_tp = &zrtp->base;
    DBGOUTI("ZRTP pjmedia_transport_zrtp_create success; nextTransport: [%s]", transport->name == NULL ? "x" : transport->name);
    return PJ_SUCCESS;
...
DEALLOC:
    ...
    if (zrtp->zrtpStartingMutex != NULL){
        pj_mutex_destroy(zrtp->zrtpStartingMutex);
        zrtp->zrtpStartingMutex = NULL;
    }

    if (zrtp->zrtpCtx != NULL) {
...
// Starting logic helper.
// Tries to enter mutex, if succeeds, starting is set to 1.
static pj_bool_t zrtp_start_try_lock(struct tp_zrtp *zrtp) {
    pj_bool_t ret = (pj_mutex_trylock(zrtp->zrtpStartingMutex) == PJ_SUCCESS) ? PJ_TRUE : PJ_FALSE;
    if (ret){
        zrtp->starting = 1;
        DBGOUTV("<zrtp_start_mutex_lock>");
    }

    return ret;
}

// Unlocks mutex. If unlock was successfull, starting is set to 0.
static pj_status_t zrtp_start_unlock(struct tp_zrtp *zrtp){
    pj_status_t ret = pj_mutex_unlock(zrtp->zrtpStartingMutex);
    if (ret == PJ_SUCCESS){
        zrtp->starting = 0;
        DBGOUTV("</zrtp_start_mutex_lock>");
    } else {
        DBGOUTW("Cannot unlock ZRTP start mutex");
    }

    return ret;
}

static pj_bool_t zrtp_is_starting(struct tp_zrtp *zrtp){ // Can be called anywhere, it is volatile. Mutex is checked anyway.
    return zrtp->starting;
}

static void zrtp_reset_isstarting(struct tp_zrtp *zrtp){ // Has to be called only when holding start mutex!
    zrtp->starting = 0;
}
...
static int32_t zrtp_sendDataZRTP(ZrtpContext* ctx, const uint8_t* data, int32_t length)
{
    ...
    pj_uint16_t* pus;
    pj_uint32_t* pui;
    pj_status_t sentStatus = PJ_FALSE;
    DBGOUTD("ZRTP sendDataZrtp; len=%" PRId32 "; data[0]={%c} ", length, length > 0 ? data[0] : 0x0);
    if ((totalLen) > MAX_ZRTP_SIZE){
    	DBGOUTI("ZRTP message too large, cannot send");
        return 0;
    }
    ...
    *(uint32_t*)(buffer+totalLen-CRC_SIZE) = pj_htonl(crc);
    ...
    sentStatus = pjmedia_transport_send_rtp(zrtp->slave_tp, buffer, totalLen);
	DBGOUTI("ZRTP sendDataZrtp postsend, status: %d started: %d", sentStatus, zrtp->started);

    return (sentStatus == PJ_SUCCESS) ? 1 : 0;
}
...
static int32_t zrtp_activateTimer(ZrtpContext* ctx, int32_t time)
{
    ...
    if(zrtp->timer_heap != NULL){
    	DBGOUTV("Going to schedule timer in time: %" PRId32 "; heap 0x%p heapID", time, zrtp->timer_heap);
    	pj_timer_heap_schedule(zrtp->timer_heap, &zrtp->timeoutEntry, &timeout);
    } else {
    	DBGOUTI("Warning! zrtp->timer_heap=NULL");
    }
#endif
...
#else
    if(zrtp->timer_heap != NULL){
    	pj_timer_heap_cancel_if_active(zrtp->timer_heap, &zrtp->timeoutEntry, 0);
    }
#endif
...
static void zrtp_srtpSecretsOn(ZrtpContext* ctx, char* c, char* s, int32_t verified)
{
    ...
    if (zrtp->userCallback != NULL)
    {
        ...
        /* check if we have postponed zrtp-hash check here */
        if (zrtp->zrtpHashCheckPostponed == PJ_TRUE)
        {
        	int32_t zrtp_status = PJMEDIA_ZRTPHASH_INVALID;
        	PJ_LOG(5,(THIS_FILE, "Postponed zrtp-hash check"));

        	/* Get peer hello zrtp-hash now, engine is initialized, should be OK */
        	int i = 0, matches=0;
        	char *zrtp_peer_hello_hash = zrtp_getPeerHelloHash(zrtp->zrtpCtx);
			if (zrtp_peer_hello_hash && *zrtp_peer_hello_hash) {
				PJ_LOG(4,(THIS_FILE, "Remote zrtp-hash from commitment (initialized): [%s]", zrtp_peer_hello_hash));

				// at least one has to match
				for(i=0; i<zrtp->peerSDPZrtpHashCount; i++){
					PJ_LOG(5,(THIS_FILE, "Stored SDP zrtp-hash [%.*s]", zrtp->peerSDPZrtpHash[i].slen, zrtp->peerSDPZrtpHash[i].ptr));
					matches += (pj_strcmp2(&(zrtp->peerSDPZrtpHash[i]), zrtp_peer_hello_hash) == 0) ? 1 : 0;
				}

				if (matches>0){
					zrtp_status |= PJMEDIA_ZRTPHASH_SDP_MATCHES_ZRTP;
				} else {
					zrtp_status &= ~PJMEDIA_ZRTPHASH_SDP_MATCHES_ZRTP;
					zrtp_status |= PJMEDIA_ZRTPHASH_MISMATCH;
				}

				free(zrtp_peer_hello_hash);
			} else {
				PJ_LOG(1,(THIS_FILE, "Cannot determine zrtp-hash from ZRTP engine, postponed"));
			}

			zrtp->zrtpHashCheckPostponed = PJ_FALSE;

			/* Update hash match result, propagate higher
			Propagate message higher only if ZRTP engine was
			successfully initialized. */
			zrtp_checkZrtpHashMatch(zrtp->zrtpCtx, zrtp_status);
...
static void zrtp_zrtpNegotiationFailed(ZrtpContext* ctx, int32_t severity, int32_t subCode)
{
    ...
    zrtp->started=0;
    zrtp->negotFailed=1;
    DBGOUTI("negotiationFailed, started: %d enabled: %d sev: %" PRId32 ", code: %" PRId32, zrtp->started, zrtp->enableZrtp, severity, subCode);
}
...
static void zrtp_zrtpNotSuppOther(ZrtpContext* ctx)
{
    ...
    zrtp->notSupportedCounter+=1;
    DBGOUTI("ZRTP not supported. started: %d; enabled: %d; zrtpPackets: %" PRIu32 "; count: %d",
    		zrtp->started, zrtp->enableZrtp, zrtp->zrtpCn, zrtp->notSupportedCounter);

    // If we fail 1 time to establish ZRTP, try it for the 3rd time, otherwise, end with it
    // retry only if some ZRTP packet was received in path. Otherwise it is likely that there
    // is no direct route between peers, so give it up.
    if (zrtp->notSupportedCounter <= 2 && zrtp->zrtpCn>0){
    	zrtp->started=0;
    	zrtp->negotFailed=1;
    	return;
    }
...
{
	struct tp_zrtp *zrtp = (struct tp_zrtp*)ctx->userData;
	return (pj_mutex_trylock(zrtp->zrtpMutex) == PJ_SUCCESS) ? 1 : 0;
}
...
static void zrtp_log(uint8_t severity, const char *obj, const char *fmt, ...){
    if (severity <= pj_log_get_level()){
        va_list arg;
        va_start(arg, fmt);
        pj_log(obj, severity, fmt, arg);
        va_end(arg);
    }
}
...
static int32_t zrtp_checkZrtpHashMatch(ZrtpContext* ctx, int32_t matchResult){
	...
	if (zrtp->userCallback != NULL)
	{
		return zrtp->userCallback->zrtp_checkZrtpHashMatch(zrtp->userCallback->userData, matchResult);
	}
	return 0;
}
...
{
    struct tp_zrtp *zrtp = (struct tp_zrtp*)tp;
    if (!zrtp_start_try_lock(zrtp)){
        PJ_LOG(4, (THIS_FILE, "ZRTP start in progress. Exiting this attempt"));
        return;
    }

    // We have start mutex here, starting was switched to 1.
    // Lets browse transports and see if ICE exists and how it is going with connection
    unsigned j = 0;
    pj_status_t status;
	...
	if (tp_info.specific_info_cnt > 0) {
		for (j = 0; j < tp_info.specific_info_cnt; ++j) {
			if (tp_info.spc_info[j].type == PJMEDIA_TRANSPORT_TYPE_ICE){
				...
				if (ii==NULL){
					PJ_LOG(5, (THIS_FILE, "ZRTP: ICE not present. Starting ZRTP right now"));
					break;
				}

                if (zrtp->iceBusy == 0) {
                    PJ_LOG(4, (THIS_FILE, "PJMEDIA_ICE; role:[%s] state_name:[%s | %d] comp_cnt:[%u]",
				...
				switch(ii->sess_state){
					case PJ_ICE_STRANS_STATE_NEGO:
						if(zrtp->iceBusy==0) {
                            zrtp->iceBusy=1;
                        }

                        zrtp_reset_isstarting(zrtp);
                        zrtp_start_unlock(zrtp);
						return;
						break;

					case PJ_ICE_STRANS_STATE_RUNNING:
						PJ_LOG(4, (THIS_FILE, "ZRTP: ICE is running properly, ready to start ZRTP"));
						break;

					case PJ_ICE_STRANS_STATE_FAILED:
						PJ_LOG(4, (THIS_FILE, "ZRTP: ICE failed definitely... ZRTP makes no sense here"));
						break;

                    case PJ_ICE_STRANS_STATE_READY:
                    case PJ_ICE_STRANS_STATE_INIT:
                    case PJ_ICE_STRANS_STATE_NULL:
                    default:
                        PJ_LOG(4, (THIS_FILE, "ZRTP: ICE state=%d", ii->sess_state));
                        break;
				}
			}
		}
	...
	zrtp->iceBusy=0;
    zrtp->negotFailed=0;
    DBGOUTD("pjmedia_transport_zrtp_startZrtp x1 , started: %d enabled: %d", zrtp->started, zrtp->enableZrtp);
    pj_assert(tp && zrtp->zrtpCtx);

    zrtp_startZrtpEngine(zrtp->zrtpCtx);
    if (zrtp->negotFailed){
    	zrtp->started = 0;
    	DBGOUTD("pjmedia_transport_zrtp_startZrtp x2 [Fail], started: %d enabled: %d", zrtp->started, zrtp->enableZrtp);
    } else {
        zrtp->started = 1;
        DBGOUTD("pjmedia_transport_zrtp_startZrtp x2 [ OK ], started: %d enabled: %d", zrtp->started, zrtp->enableZrtp);
    }
...
PJ_DEF(void) pjmedia_transport_zrtp_stopZrtp(pjmedia_transport *tp)
{
    ...
    DBGOUTI("pjmedia_transport_zrtp_stopZrtp, started: %d enabled: %d", zrtp->started, zrtp->enableZrtp);
}
...
PJ_DECL(pjmedia_transport *) pjmedia_transport_zrtp_getSlaveTransport(pjmedia_transport *tp){
	struct tp_zrtp *zrtp = (struct tp_zrtp*)tp;
	PJ_ASSERT_RETURN(tp, NULL);

	return zrtp->slave_tp;
}
                                      ...
                                      pjmedia_transport_info *info)
{
    ...
    PJ_ASSERT_RETURN(sizeof(pjmedia_zrtp_info) <= PJMEDIA_TRANSPORT_SPECIFIC_INFO_MAXSIZE, PJ_ETOOBIG);

    zrtp_info.active = zrtp_inState(zrtp->zrtpCtx, SecureState) ? PJ_TRUE : PJ_FALSE;
    // misc helper fields
    zrtp_info.negotFailed = zrtp->negotFailed;
    zrtp_info.starting    = zrtp->starting;
    zrtp_info.iceBusy     = zrtp->iceBusy;
    zrtp_info.peerSDPZrtpHashesSeen = zrtp->peerSDPZrtpHashesSeen;
    zrtp_info.zrtpHashCheckPostponed = zrtp->zrtpHashCheckPostponed;
    zrtp_info.notSupportedCounter = zrtp->notSupportedCounter;
    zrtp_info.rtpUnsec 	  = zrtp->rtpUnsec;
    zrtp_info.rtpSec      = zrtp->rtpSec;
    zrtp_info.zrtpCn      = zrtp->zrtpCn;
    zrtp_info.protect     = zrtp->protect;
    zrtp_info.unprotect   = zrtp->unprotect;

    // find current ZRTP state, no direct method here, has to iterate
    for(zrtp_info.zrtpState=0; zrtp_info.zrtpState < numberOfStates; zrtp_info.zrtpState++){
    	if (zrtp_inState(zrtp->zrtpCtx, zrtp_info.zrtpState)) break;
    }
...
static void transport_rtp_cb(void *user_data, void *pkt, pj_ssize_t size)
{
    ...
    if ((*buffer & 0xf0) != 0x10)
    {
        ...
        {
        	// RTP packet received && not secure mode
        	zrtp->rtpUnsec++;
            zrtp->stream_rtp_cb(zrtp->stream_user_data, pkt, size);
        }
        else
        {
        	// RTP packet received, decrypting ZSRTP packet
        	zrtp->rtpSec++;
        ...
        if (!zrtp->started && zrtp->enableZrtp){
        	DBGOUTI("Starting ZRTP transport 984");
    ...
    if (zrtp->enableZrtp && zrtp->zrtpCtx != NULL)
    {
        ...
        DBGOUTI("ZRTP packet received: %" PRIu32, zrtp->zrtpCn);

        // not to cause deadlock
        if (zrtp_is_starting(zrtp)){
            PJ_LOG(4, (THIS_FILE, "ZRTP start in progress. Ignoring ZRTP packet"));
            return;
        }
        ...
        {
        	DBGOUTW("No ZRTP magic packet || null context");
            return;
        }
        ...
        if (!zrtp->started)
        {
        	DBGOUTI("Starting ZRTP transport 1020");
            pjmedia_transport_zrtp_startZrtp((pjmedia_transport *)zrtp);
        }

        // if ICE is busy right now, we cannot send reply, so disable ZRTP processing
        if(zrtp->iceBusy){
        	DBGOUTI("ICE is busy, cannot react on this ZRTP packet, ignoring it");
        	return;
        }
        ...
        DBGOUTI("going to process ZRTP message with engine");
        zrtp_processZrtpMessage(zrtp->zrtpCtx, zrtpMsg, zrtp->peerSSRC, size);
    } else {
    	DBGOUTW("ZRTP received but ZRTP not enabled");
    }
}
                                                    ...
                                                    pj_ssize_t))
{
    ...
    {
    	DBGOUTI("attaching ZRTP transport failed, setting null callbacks");
    ...
    DBGOUTI("attaching ZRTP transport passed");
    return PJ_SUCCESS;
}
...
static void transport_detach(pjmedia_transport *tp, void *strm)
{
    ...
    DBGOUTI("Detaching ZRTP transport");
                                      ...
                                      pj_size_t size)
{
    ...
    //PJ_LOG(4, (THIS_FILE, "transport_send_rtp, started: %d enabled: %d", zrtp->started, zrtp->enableZrtp));
    if (!zrtp->started && zrtp->enableZrtp)
    {
        if (zrtp->localSSRC == 0)
            zrtp->localSSRC = pj_ntohl(pui[2]);   /* Learn own SSRC before starting ZRTP */

        DBGOUTI("Starting ZRTP transport 1153");
        pjmedia_transport_zrtp_startZrtp((pjmedia_transport *)zrtp);
    }

    // seeding a ZRTP random number generator a bit from RTP packet being sent
    if (pkt!=NULL && size > 8 && (rand() % 50) == 1){
    	zrtp_addEntropy((char *) pkt, size < 32 ? size : 32);
    }

    // if SRTP is not yet initialized, packet is sent by a slave transport.
    if (zrtp->srtpSend == NULL)
    {
    	//PJ_LOG(4, (THIS_FILE, "transport_send_rtp1, started: %d enabled: %d", zrtp->started, zrtp->enableZrtp));
    ...
    else
    {
        ...
        //PJ_LOG(4, (THIS_FILE, "transport_send_rtp2, started: %d enabled: %d", zrtp->started, zrtp->enableZrtp));
        if (rc == 1)
                                        ...
                                        unsigned media_index)
{
	...
	for (i = 0; i < numVersions; i++) {
		...
		if (zrtp_hello_hash && *zrtp_hello_hash) {
			...
			zrtp_hash = pjmedia_sdp_attr_create(sdp_pool, ZRTP_HASH_STRING, zrtp_hash_str);
			if (zrtp_hash &&
...
/**
 * Extracts all attributes from given destination and store pointers on them to the array
 * @param destAttrArr is an array of pointers to SDP attributes
 */
static pj_status_t extract_all_attributes(
		pjmedia_sdp_attr **destAttrArr,
		unsigned *addedCount,
		unsigned maxLen,
		unsigned count,
	...
	pj_str_t name;						// pj_str_t representation of a header name
	pjmedia_sdp_attr *cur_attr = NULL;	// current attribute found
	unsigned cur_count = count;			// current size of the array of attributes to search
	unsigned curIdx = 0;				// index where array to search starts in the base array

	PJ_ASSERT_RETURN(destAttrArr!=NULL && addedCount!=NULL, PJ_EINVAL);
	if (maxLen<=0 || attr_array==NULL || count <=0){
		return PJ_SUCCESS;
	}

	// create pj_str_t representation of header to find
	name.ptr = (char*)c_name;
	name.slen = pj_ansi_strlen(c_name);

	// Idea: find the first matching attribute, then find another in the rest of attribute array, ...
	// Iterate until a) array is not empty, b) there is space in result array
	while(cur_count > 0 && (*addedCount) < maxLen) {
		// find next matching attribute
		cur_attr = pjmedia_sdp_attr_find(cur_count, &(attr_array[curIdx]), &name, c_fmt);
		if (cur_attr==NULL) {
			break;
		}

		// add this found attribute to the return array
		destAttrArr[(*addedCount)++] = cur_attr;
		// get index of the next attribute of the returned one
		for(; curIdx < count && (attr_array[curIdx] != cur_attr); curIdx++);
		curIdx+=1;
		// new count of attribute array, subtract
		cur_count = count - curIdx;
		if (cur_count<=0){
			break;
		}
	}
...
{
    struct tp_zrtp *zrtp = (struct tp_zrtp*)tp;
    pjmedia_sdp_attr *zrtp_hash[MAX_ZRTP_VERSIONS];
    int zrtp_hash_count = 0;
    int zrtp_status = PJMEDIA_ZRTPHASH_INVALID;
    int i;
    ...
    if (rem_sdp){
    	zrtp->zrtpHashCheckPostponed = PJ_FALSE;
    	PJ_LOG(4,(THIS_FILE, "transport_media_start, going to examine zrtp-hash"));

    	/* At first extract zrtp-hash from ZRTP engine.
    	 * SDP session is established, so we should have zrtp-hash from SDP,
    	 * but ZRTP engine may not be initialized yet. If it is the case
    	 * then we have to copy zrtp-hash from SDP to context and postpone
    	 * check for later, SAS stage (zrtp initialized).
    	 */
		char *zrtp_peer_hello_hash = zrtp_getPeerHelloHash(zrtp->zrtpCtx);
		if (zrtp_peer_hello_hash && *zrtp_peer_hello_hash) {
			PJ_LOG(4,(THIS_FILE, "Remote zrtp-hash from commitment: [%s]", zrtp_peer_hello_hash));

		} else {
			PJ_LOG(3,(THIS_FILE, "Cannot determine zrtp-hash from ZRTP engine, postponed to SAS section"));

			/* set check state to postponed */
			zrtp_status |= PJMEDIA_ZRTPHASH_NO_ZRTP;
			zrtp_peer_hello_hash = NULL;
		}


		/*
		 * Now obtain zrtp-hash from SDP session
		 */
		extract_all_attributes(&(zrtp_hash[0]), &zrtp_hash_count, MAX_ZRTP_VERSIONS,
				rem_sdp->attr_count,
				rem_sdp->attr,
				ZRTP_HASH_STRING,
				NULL);
		if (zrtp_hash_count==0){
			PJ_LOG(4,(THIS_FILE, "Cannot determine zrtp-hash from remote SDP session; attr count=%d; mediacnt=%d; media_idx=%d, media[idx]=%p",
								rem_sdp->attr_count, rem_sdp->media_count, media_index, rem_sdp->media[media_index]));
		}

		// inspect media SDP attributes
		if (zrtp_hash_count < MAX_ZRTP_VERSIONS){
			PJ_ASSERT_RETURN(media_index < rem_sdp->media_count,  PJ_EINVAL);
		    PJ_ASSERT_RETURN(rem_sdp->media[media_index] != NULL, PJ_EINVAL);
			extract_all_attributes(&(zrtp_hash[zrtp_hash_count]), &zrtp_hash_count, MAX_ZRTP_VERSIONS-zrtp_hash_count,
					rem_sdp->media[media_index]->attr_count,
					rem_sdp->media[media_index]->attr,
					ZRTP_HASH_STRING,
					NULL);
		}

		if (zrtp_hash_count == 0){
			PJ_LOG(1,(THIS_FILE, "Cannot determine zrtp-hash from remote SDP session; unable to check against committed"));
			zrtp_status |= PJMEDIA_ZRTPHASH_NO_SDP;

		} else {
			zrtp->peerSDPZrtpHashesSeen = zrtp_hash_count;
			for(i=0; i<zrtp_hash_count; i++){
				PJ_LOG(4,(THIS_FILE, "Remote zrtp-hash from remote SDP session: [%s]", zrtp_hash[i]->value));
			}

			// SDP is OK, check if we have ZRTP and whether we can do check now
			if (zrtp_peer_hello_hash != NULL){
				// at least one has to match
				int matches=0;
				for(i=0; i<zrtp_hash_count; i++){
					matches += (pj_strcmp2(&(zrtp_hash[i]->value), zrtp_peer_hello_hash) == 0) ? 1 : 0;
				}
				...
				// check is postponed for later, copy SDP zrtp-hash to context
				zrtp->zrtpHashCheckPostponed = PJ_TRUE;
				zrtp->peerSDPZrtpHashCount   = zrtp_hash_count;
				for(i=0; i<zrtp_hash_count; i++){
					pj_strdup(zrtp->pool, &(zrtp->peerSDPZrtpHash[i]), &(zrtp_hash[i]->value));
				}
			}
		}

		// Free zrtp-hash from ZRTP if any. It is allocated by malloc().
		if (zrtp_peer_hello_hash!=NULL){
			free(zrtp_peer_hello_hash);
		}
    }
    ...
    /* Update hash match result, propagate higher
       Propagate message higher only if ZRTP engine was
       successfully initialized. */
    if (zrtp->zrtpHashCheckPostponed == PJ_FALSE){
    	zrtp_checkZrtpHashMatch(zrtp->zrtpCtx, zrtp_status);
    }
...
static pj_status_t transport_media_stop(pjmedia_transport *tp)
{
    ...
    /* Do something.. */
    PJ_LOG(4, (THIS_FILE,
    		"Media stop - encrypted packets: %" PRIu64 ", decrypted packets: % " PRIu64 " "
    		"RTP recv secured/unsecured: [%" PRIu64 "/% " PRIu64 "]; "
    		"ZRTP cnt: %" PRIu32,
            zrtp->protect, zrtp->unprotect, zrtp->rtpSec, zrtp->rtpUnsec, zrtp->zrtpCn));
...
static pj_status_t transport_destroy(pjmedia_transport *tp)
{
    ...
    PJ_LOG(4, (THIS_FILE,
    		"Destroy - encrypted packets: %" PRIu64 ", decrypted packets: %" PRIu64 ", "
    		"RTP recv secured/unsecured: [%" PRIu64 "/%" PRIu64 "]; "
    		"ZRTP cnt: %" PRIu32 "",
            zrtp->protect, zrtp->unprotect, zrtp->rtpSec, zrtp->rtpUnsec, zrtp->zrtpCn));
    ...
    if (zrtp->zrtpStartingMutex != NULL) {
        pj_mutex_lock(zrtp->zrtpStartingMutex);
        pj_mutex_unlock(zrtp->zrtpStartingMutex);
        pj_mutex_destroy(zrtp->zrtpStartingMutex);
        zrtp->zrtpStartingMutex = NULL;
    }
    ...
    /* Inform zrtp user this transport is about to destroy. */
    if (zrtp->userCallback != NULL){
        zrtp->userCallback->zrtp_transportDestroy(zrtp->userCallback->userData);
    }
