...
def add_pjsua_rename(text):
    folder = os.path.dirname(os.path.realpath(__file__))
    nonsync_methods_file = open(os.path.join(folder, "non_synchronized_methods.txt"))
    nonsync_methods = [x.strip() for x in nonsync_methods_file.readlines()]
    ...
    for m in regex.finditer(text):
        ...
        for nonsync_method in nonsync_methods:
            if len(nonsync_method) > 0 and not nonsync_method.startswith("#"):
                nonsync_regex = re.compile(nonsync_method, re.MULTILINE|re.VERBOSE|re.DOTALL)
                if nonsync_regex.match(method_line):
                    should_sync = False
                    break

        if should_sync:
            with_decls.append("%%rename(%s) %s;\n%%javamethodmodifiers %s \"public synchronized\";\n" % ( m.group(3) , m.group(2), m.group(1) ) )
        else:
            with_decls.append("%%rename(%s) %s;\n" % ( m.group(3) , m.group(2) ) )
