...
void on_reg_started2_wrapper(pjsua_acc_id acc_id, pjsua_reg_info *info){
	registeredCallbackObject->on_reg_started2(acc_id, info);
}
...
void on_transport_state_wrapper(pjsip_transport *tp, pjsip_transport_state state, const pjsip_transport_state_info *info) {
	return registeredCallbackObject->on_transport_state(tp, state, info);
}

void on_ice_transport_error_wrapper(int index, pj_ice_strans_op op, pj_status_t status, void *param){
	registeredCallbackObject->on_ice_transport_error(index, op, status);
}

pj_status_t on_snd_dev_operation_wrapper(int operation){
	registeredCallbackObject->on_snd_dev_operation(operation);
	return PJ_SUCCESS;
}

void on_call_media_event_wrapper(pjsua_call_id call_id, unsigned med_idx, pjmedia_event *event){
	registeredCallbackObject->on_call_media_event(call_id, med_idx, event);
}

void on_reregistration_compute_backoff_wrapper(pjsua_acc_id acc_id, pj_reg_backoff_struct * backoff_struct) {
	return registeredCallbackObject->on_reregistration_compute_backoff(acc_id, backoff_struct);
}
...
struct pjsua_callback wrapper_callback_struct = {
	...
	NULL, // on_reg_started
	&on_reg_started2_wrapper, // on_reg_started2
	&on_reg_state_wrapper,
	&on_reg_state2_wrapper, //on_reg2_state
	...
	&on_typing_wrapper,
	&on_typing2_wrapper, //Typing 2
	&on_nat_detect_wrapper,
	&on_call_redirected_wrapper,
	NULL, //on_mwi_state
	&on_mwi_info_wrapper,
	&on_transport_state_wrapper, //on_transport_state
	NULL, //on_call_media_transport_state
	&on_ice_transport_error_wrapper, //on_ice_transport_error
	&on_snd_dev_operation_wrapper, //on_snd_dev_operation
	&on_call_media_event_wrapper, //on_call_media_event
	NULL, //on_create_media_transport
    &on_acc_find_for_incoming_wrapper,
	NULL, //on_stun_resolution_complete,
	&on_reregistration_compute_backoff_wrapper,
};
