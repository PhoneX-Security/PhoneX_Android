...
static jmethodID reportToAcraMethod;
...
void reportToAcra(const char *reason) {
    JNIEnv *env = 0;
	int result = javaVM->GetEnv((void **) &env, JNI_VERSION_1_6);

	if (result == JNI_EDETACHED) {
    	__android_log_print(ANDROID_LOG_WARN, "NativeCrashHandler", "reportToAcra: Native crash occured in a non jvm-attached thread");
        result = javaVM->AttachCurrentThread(&env, NULL);
    }

    if (result != JNI_OK){
    	__android_log_print(ANDROID_LOG_ERROR, "NativeCrashHandler", "reportToAcra: Could not attach thread to Java VM for crash reporting.\n"
    			"Crash was: %s", reason);
    } else if (env && applicationObject) {
        // method call itself
        env->CallVoidMethod(applicationObject, reportToAcraMethod, env->NewStringUTF(reason));
    	Verify(env->ExceptionCheck() == JNI_FALSE, "reportToAcra: Java threw an exception");
    } else {
        __android_log_print(ANDROID_LOG_ERROR, "NativeCrashHandler",
        				"reportToAcra: Could not create native crash report as registerForNativeCrash was not called in JAVA context.\n"
        				"Crash was: %s", reason);
    }
}
...
void nativeCrashHandler_onLoad(JavaVM *jvm) {
	...
	reportToAcraMethod = env->GetMethodID(applicationClass, "reportToAcra", "(Ljava/lang/String;)V");
    Verify(reportToAcraMethod, "Could not find reportToAcra java method");
