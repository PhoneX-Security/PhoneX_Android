...
*/
	...
	if (stream->dir & PJMEDIA_DIR_CAPTURE) {
		...
		PJ_LOG(3, (THIS_FILE, "Creating a new object-2: source: %d, clock rate: %d, channel config: %d, sample format: %d, buffsize: %d",
                                        mic_source, param->clock_rate, 2, sampleFormat, inputBuffSizeRec));
		...
		if (exc) {
			...
			if(mic_source != 1){
				...
				PJ_LOG(3, (THIS_FILE, "Creating a new object-3: source: %d, clock rate: %d, channel config: %d, sample format: %d, buffsize: %d",
                                                        1, param->clock_rate, 2, sampleFormat, inputBuffSizeRec));
				stream->record =  jni_env->NewObject(stream->record_class, constructor_method,
		...
		if(state == 0){ /* STATE_UNINITIALIZED */
			...
			if(mic_source != 1){
				PJ_LOG(4, (THIS_FILE, "Try default source"));
				PJ_LOG(3, (THIS_FILE, "Creating a new object-4: source: %d, clock rate: %d, channel config: %d, sample format: %d, buffsize: %d",
                                                        1, param->clock_rate, 2, sampleFormat, inputBuffSizeRec));
				stream->record =  jni_env->NewObject(stream->record_class, constructor_method,
							1, // Mic input source:  1 = MIC / 7 = VOICE_COMMUNICATION
							param->clock_rate,
	...
	if (stream->dir & PJMEDIA_DIR_PLAYBACK) {
		...
		PJ_LOG(3, (THIS_FILE, "Creating a new object-5: source: %d, clock rate: %d, channel config: %d, sample format: %d, buffsize: %d",
                                        0, param->clock_rate, 2, sampleFormat, inputBuffSizePlay));
