...
do
  if [ -d $ANDROID_NDK/toolchains/arm-linux-androideabi-4.8/prebuilt/$i-x86_64/bin ]; then
    PATH=$ANDROID_NDK/toolchains/arm-linux-androideabi-4.8/prebuilt/$i-x86_64/bin:$PATH
  fi
done
