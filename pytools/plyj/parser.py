...
class StatementParser(object):
    ...
    def p_method_invocation(self, p):
        ...
        p[0] = MethodInvocation(p[1], arguments=p[3], linespan=self.get_max_linespan(p), lexspan=self.get_max_lexspan(p))
    ...
    def p_method_invocation2(self, p):
        ...
        p[0] = MethodInvocation(p[4], target=p[1], type_arguments=p[3], arguments=p[6], linespan=self.get_max_linespan(p), lexspan=self.get_max_lexspan(p))
    ...
    def p_method_invocation3(self, p):
        ...
        #if p[1]=="Log" or (isinstance(p[1], Name) and p[1].value=="Log"):
        #    print p.lexspan(2)
        p[0] = MethodInvocation(p[3], target=p[1], arguments=p[5], linespan=self.get_max_linespan(p), lexspan=self.get_max_lexspan(p))
