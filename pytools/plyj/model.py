...
class BinaryExpression(Expression):
    ...
    def accept(self, visitor):
        if visitor.visit_BinaryExpression(self): 
            if self.lhs!=None and not isinstance(self.lhs, str): 
                self.lhs.accept(visitor)
            if self.rhs!=None and not isinstance(self.rhs, str):
...
class MethodInvocation(Expression):
    def __init__(self, name, arguments=None, type_arguments=None, target=None, linespan=None, lexspan=None):
        super(MethodInvocation, self).__init__()
        self._fields = ['name', 'arguments', 'type_arguments', 'target', 'linespan', 'lexspan']
        if arguments is None:
            arguments = []
        if type_arguments is None:
            type_arguments = []
